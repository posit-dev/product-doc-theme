...
From this version, GTM manages cookie consent instead of the theme.
Contact WebOps before you upgrade, so that they can opt your site in for tracking.
...
- (Breaking) Automatically configure analytics.
  Projects no longer need a copy of `_analytics.html` or the `analytics.google` entry in `_variables.yml`
...
- (Breaking) Remove `posit-docs-html` and have the `posit-docs` project type directly modify the `html` format
